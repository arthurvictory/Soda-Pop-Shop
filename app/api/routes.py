...
def create_contact(current_user_token):
    ...
    soda = request.json['soda']
    flavor = request.json['flavor']
    user_token = current_user_token.token

    print(f'BIG TESTER: {current_user_token.token}')

    contact = Contact(name, email, phone_number, address, year, soda, flavor, user_token = user_token )
...
def update_contact(current_user_token, id):
    ...
    contact.soda = request.json['soda']
    contact.flavor = request.json['flavor']
...
@token_required
def delete_contact(contact_user_token, id):
    contact = Contact.query.get(id)
